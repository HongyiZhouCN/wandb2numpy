...
def read_tt_data(data_path):
    ...
    for subdict in subdict_list:
        if "evaluation_is_success.npy" in os.listdir(subdict):
            ...
            is_success = delete_jth_dim(is_success, 0)
            is_success_list.append(is_success)

        if "evaluation_hit_ball.npy" in os.listdir(subdict):
            hitting = np.load(subdict + "/evaluation_hit_ball.npy")
            hitting = delete_jth_dim(hitting, 0)
            hitting_list.append(hitting)

        if "evaluation_mean.npy" in os.listdir(subdict):
            ep_reward = np.load(subdict + "/evaluation_mean.npy")
            ep_reward = delete_jth_dim(ep_reward, 0)
        ...
        if "simulation_steps.npy" in os.listdir(subdict):
            ...
            simulation_steps = delete_jth_dim(simulation_steps, 0)
...
def draw_bp_iqm(is_success, simulation_steps, algorithm, method, label):
    fig, ax = plt.subplots(ncols=1, figsize=(7, 5))
    num_frame = 35
    frames = np.floor(np.linspace(1, is_success.shape[-1], num_frame)).astype(
        int) - 1
...
if __name__ == "__main__":
    ...
    data_path = f"/home/hongyi/Codes/bruce_iclr/wandb2numpy/wandb_data/TTrandInit_trpl_20seeds"
    # is_success, simulation_steps = read_bp_data(data_path)
